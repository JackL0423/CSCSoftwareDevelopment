...
class _SearchPageWidgetState extends State<SearchPageWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      logFirebaseEvent('SearchPage_custom_action');
      await actions.initializeUserSession();
      logFirebaseEvent('SearchPage_firestore_query');
      _model.recipesList = await queryRecipesRecordOnce(
        queryBuilder: (recipesRecord) => recipesRecord.orderBy('timestamp'),
        limit: 5,
      );
      logFirebaseEvent('SearchPage_update_app_state');
      FFAppState().SearchPageVisitCount = FFAppState().SearchPageVisitCount + 1;
      safeSetState(() {});
      if ((FFAppState().SearchPageVisitCount.toString() == '3') &&
          (FFAppState().surveyCompleted == true)) {
        logFirebaseEvent('SearchPage_wait__delay');
        await Future.delayed(
          Duration(
            milliseconds: 5000,
          ),
        );
        logFirebaseEvent('SearchPage_navigate_to');

        context.pushNamed(NPSSurveyWidget.routeName);
      }
    });
                        ...
                        return ListView.separated(
                          ...
                          itemBuilder: (context, listViewIndex) {
                            ...
                            return RecipePreviewWidget(
                              key: Key(
                                  'Keynvm_${listViewIndex}_of_${listViewRecipesRecordList.length}'),
                              img: listViewRecipesRecord.img,
                              title: listViewRecipesRecord.title,
                              shortDesc: listViewRecipesRecord.desc,
                              difficulty: listViewRecipesRecord.difficulty,
                              prepTime: listViewRecipesRecord.prep,
                              region: listViewRecipesRecord.region,
                            );
                          },
                        );
                      },
